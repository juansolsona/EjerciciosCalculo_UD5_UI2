...
\newfontfamily\subsectionfont{Verdana}[FakeBold=1.6,LetterSpace=1.5]
...
\titleformat{\subsection}
  {\subsectionfont\large\bfseries}   % subsecciones más bold
  {\thesubsection}{0.8em}{}
...
\begin{center}
  \small © 2025 Juan Manuel Solsona Sagrado \\
  \small © 2025 Josè Luis Salazar \\
  \small © 2025 Josè David Gòmez
...
\section{Demostrar las fórmulas utilizadas para el gradiente cuando se usa la función sigmoide}
...
\subsection{Definamos primero cada elemento}
...
\subsubsection{Derivada del modelo de datos}
...
\text{Cada término para calcular el límite es:}
...
\text{Con lo que la expresión completa es:}
...
\subsubsection{Derivada de la función sigmoide}

\[
f(z) = \frac{1}{1 + e^{-z}}
\]

\text{Definición de derivada:}
...
\subsubsection{Derivada de la función de error}
...
\text{Ahora podemos aplicar la regla de la cadena, y las derivadas que nos quedan son:}
...
\section{Modelado de los datos usando una única neurona}
...
\subsection{Normalizar los datos}
...
\begin{tabular}{|c|c|c|}
\hline
\text{Lote} & \text{Edad Normalizada} & \text{Ingresos Normalizados} \\
\hline
1  & 0.5263 & 0.8750 \\
2  & 0.0789 & 0.1875 \\
3  & 0.8684 & 0.3125 \\
...
Tambien es posible mostrar graficamente la evolucion de los pesos y los grandientes en el proceso de entrenamiento:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{excel/G1.png}
    \caption{Evolución de los pesos durante el entrenamieno}
    \label{fig:miimagen}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{excel/G2.png}
    \caption{Evolución de los gradientes durante el entrenamieno}
    \label{fig:miimagen}
\end{figure}

\subsection{Tabla detallada para cada lote $t$ de las constantes antes y después de utilizar el lote, así como los gradientes, para los 10 primeros lotes (2.5 puntos).}
...
\hline
\text{Lote} & \text{$x_1$} & \text{$x_2$} & \text{$y$} &
\text{$w_1$} & \text{$w_2$} & \text{$b$} &
\text{$h$} & \text{$\hat y$} &
\text{$\partial e/\partial w_1$} &
\text{$\partial e/\partial w_2$} &
\text{$\partial e/\partial b$} \\
\hline
...
\subsection{Predicción para los lotes 11 a 14 (1 punto).}
...
A partir de este punto, esos valores quedan fijos, ya que no se pide continuar el entrenamiento.

\subsubsection{Lote 11}
...
\subsection{Hacer una propuesta de otras funciones de activación que consideréis interesantes
...
\subsubsection{Función Softsign}
\[
a(z) = \frac{z}{1 + |z|}
\]
...
\subsection{Extensión del modelo con varias neuronas}
...
\subsubsection{Capa oculta con tres neuronas}
...
\subsubsection{Interpretación de las neuronas ocultas}

Cada neurona de la capa oculta puede interpretarse como una “región” o “perfil de cliente”
en el plano edad–ingresos:

\begin{itemize}
  \item Una neurona puede especializarse en detectar \text{ingresos altos}
  (peso $w_{2j}$ grande y $w_{1j}$ pequeño).
  \item Otra neurona puede detectar \text{edad media o alta}
...
\section{Modelado de datos usando varias neuronas}

\subsection{Calcular las fórmulas que faltan del gradiente}

\subsubsection{Comprobar $\frac{\partial e}{\partial w_{11}}$}
...
\text{Aplicación de la regla de la cadena}
...
\text{1. Derivada del error respecto a la predicción}
...
\text{3. Derivada del modelo lineal de la neurona de salida}
...
\text{4. Derivada de la sigmoide de la neurona oculta}
...
\text{5. Derivada del modelo lineal de la neurona oculta}
...
\subsubsection{Comprobar $\frac{\partial e}{\partial w_{12}}$}
...
\subsubsection{Comprobar $\frac{\partial e}{\partial w_{21}}$}
...
\subsubsection{Comprobar $\frac{\partial e}{\partial w_{22}}$}
...
\subsubsection{Comprobar $\frac{\partial e}{\partial w_{31}}$}
...
\subsubsection{Comprobar $\frac{\partial e}{\partial w_{32}}$}
...
\subsubsection{Resultado general para cualquier peso $w_{ij}$ de la capa oculta}
...
\subsection{Tabla detallada para cada lote $t$ de las constantes antes y después de utilizar el lote, así como los gradientes, para los 10 primeros lotes}

Realizar este ejercicio en Microsoft Excel haría una tabla realmente complicada de definir, así que he optado por una solución programática, en Python, sin usar ninguna librería específica para redes neuronales. 
El código completo se puede revisar en el Anexo 2.
...
\subsection{Predicción para los lotes 11 a 14}
